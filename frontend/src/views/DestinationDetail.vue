    ...
    <template v-else>
      <div class="detail-hero" :style="{ backgroundImage: `url(${item.image})` }">
        ...
        <div class="detail-hero-content">
          ...
          <h1>{{ item.name }}</h1>
          <div class="hero-meta">
            <span class="meta-item" v-if="item.province || item.location">
        ...
        <div class="main-content">
          ...
          <div class="loc-section" v-if="mapQuery">
            ...
            <div class="location-detail">
              ...
              <div>
                <span class="loc-main">{{ item.location }}{{ item.province ? ', ' + item.province : '' }}</span>
                <span class="loc-province">South Africa</span>
              </div>
            </div>
        ...
        <div class="sidebar">
          <div class="sidebar-card sticky">
            ...
            <div class="sidebar-details">
              <div class="detail-row" v-if="item.hours">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
              ...
              <div class="detail-row">
                ...
                <span>{{ item.location }}{{ item.province ? ', ' + item.province : '' }}</span>
              </div>
...
const item = computed(() => {
  const d = destMap.value[route.params.id]
  if (!d) return null
  return { ...d, image: provinceImages[d.name] || d.image }
})

const mapQuery = computed(() => {
  if (!item.value || !item.value.location) return ''
  const parts = [item.value.location, item.value.province, 'South Africa'].filter(Boolean)
  return encodeURIComponent(parts.join(', '))
})

function openItineraryFor(destItem) {
  selectedForItinerary.value = {
    title: destItem.name,
    location: destItem.location,
    province: destItem.province || '',
    description: destItem.description || '',
    price: destItem.priceRange ? parseFloat(destItem.priceRange.replace(/[^0-9.]/g, '')) || 0 : 0,
    duration_hours: null,
    id: null,
  }
...
.price-val { display: block; font-size: 1.1rem; font-weight: 700; font-family: 'Poppins', sans-serif; color: var(--accent); }
